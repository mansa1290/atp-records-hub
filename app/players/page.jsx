...
export default function PlayersPage() {
  ...
  if (loading) return <div className="p-8 text-center">Loading top players...</div>;
  if (error) return <div className="p-8 text-center text-red-500">Error: {error}</div>;

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-8">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-5xl font-bold text-white mb-2 gradient-text">Top ATP Players</h1>
            ...
            <Link key={player.player_id} href={`/players/${player.player_id}`}>
              <div className="bg-slate-700/50 border border-slate-600 rounded-lg p-4 hover:border-cyan-500 hover:shadow-lg hover:shadow-cyan-500/20 transition cursor-pointer">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-lg font-bold text-white truncate">{player.name || 'Unknown'}</h3>
                  <span className="text-cyan-400 font-bold text-sm">#{idx + 1}</span>
                </div>
                <p className="text-slate-400 text-sm mb-2">ID: {player.player_id}</p>
                <p className="text-slate-400 text-sm">Rank: {player.rank || 'N/A'}</p>
              </div>
            </Link>
          ))}
        </div>
